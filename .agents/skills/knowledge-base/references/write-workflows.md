...
7. 运行 `python3 scripts/kb.py wiki-candidates --note-id "NOTE_ID"`，读取新笔记的 `related` 和候选页面。
8. 做一次语义影响检查并明确记录结果：
   - 命中已有主题／实体页：读取该页实际来源，更新仍有效的结论、证据、矛盾和待确认问题；
   - 只有关联候选：先核对主题是否真的一致，再决定更新，不把“感觉相关”直接写入；
   - 没有实体页：由 AI 检查核心人物、公司、产品或工具；至少两条有效笔记才建正式实体页，单条资料只有用户明确要求时才建 `draft`；
   - 不满足条件时报告“主题／实体：暂无（原因）”，不为了凑齐编译层而创建页面。
9. 语义页面写入或确认不变后，再次运行 `python3 scripts/kb.py sync`。
10. 交付结果必须分别报告：原文、原子笔记、主题 Wiki、实体 Wiki 的新增／更新／候选／跳过／失败及原因。

## 收录本地文件

1. 确认文件可读，运行 `hash-file` 和 `lookup --content-hash` 查重。
2. 将原文件复制到 `raw/YYYY/MM/`，保留扩展名和内容，不覆盖同名文件。
3. 读取可提取内容并按「收录链接」第 5—9 步创建笔记。PDF 没有可提取文本时停止；OCR 不在 MVP 范围。
...
4. 运行 `sync` 和 `wiki-candidates`，再按需要回织主题／实体 Wiki；最终再次运行 `sync`。

## 编译主题或实体

1. 先用 `search`、`INDEX.md`、`wiki-candidates` 和笔记的 `related` 找齐相关笔记。
...
5. 实体页只为核心人物、公司、产品、工具或组织建立；普通术语优先放在主题页。更新已有页时保留仍然有效的矛盾和来源，刷新 `updated_at`，再运行 `sync`。
